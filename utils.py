...
def collate_fn(sign2id, batch):
  ...
  training_target = formulas2tensor(add_start_token(formulas), sign2id)
  # targets for calculating loss , end with END_TOKEN
  cal_loss_target = formulas2tensor(add_end_token(formulas), sign2id)
  imgs = torch.stack(imgs, dim=0)
  return imgs, training_target, cal_loss_target
...
def load_formulas(fname):
  formulas = dict()
  with open(fname) as f:
    for idx, line in enumerate(f):
      formulas[idx] = line.strip()
  print("Loaded {} formulas from {}".format(len(formulas), fname))
  return formulas
...
def schedule_sample(prev_logit, prev_tgt, epsilon):
  out = torch.argmax(prev_logit, dim=1, keepdim=True)
  choices = torch.cat([out, prev_tgt], dim=1)  # [B, 2]
  batch_size = choices.size(0)
  prob = Bernoulli(torch.tensor([epsilon]*batch_size).unsqueeze(1))
  # sampling
  sample = prob.sample().long().to(prev_tgt.device)
  next_inp = torch.gather(choices, 1, sample)
  return next_inp
