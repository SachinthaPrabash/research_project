...
def score_files(path_ref, path_hyp):
  ...
  # score
  return {"BLEU-4": bleu_score(refs, hyps)*100, "EM": exact_match_score(refs, hyps)*100, "Edit": edit_distance(refs, hyps)*100
  }
...
def exact_match_score(references, hypotheses):
  ...
  for ref, hypo in zip(references, hypotheses):
    if np.array_equal(ref, hypo):
        match_n += 1
  return match_n / float(max(len(hypotheses), 1))
...
def edit_distance(references, hypotheses):
  ...
  """
  levenshtein_distance, total_length = 0, 0
  for ref, hypo in zip(references, hypotheses):
    levenshtein_distance += distance.levenshtein(ref, hypo)
    total_length += float(max(len(ref), len(hypo)))
