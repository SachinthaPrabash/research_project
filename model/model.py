...
class Im2LatexModel(nn.Module):
  def __init__(self, out_size, emb_size, dec_rnn_h, enc_out_dim=512,  n_layer=1, add_pos_feat=False, dropout=0.):
    ...
    self.rnn = nn.LSTMCell(dec_rnn_h+emb_size, dec_rnn_h)
  ...
  def forward(self, imgs, formulas, epsilon=1.):
    ...
    """
    # encoding
    images = self.encode(imgs)  # [B, H*W, 512]
    # init decoder's states
    states, o_t = self.init_decoder(images)
    ...
    for t in range(max_len):
      ...
      # ont step decoding
      states, O_t, logit = self.step_decoding(states, o_t, images, tgt)
      logits.append(logit)
    logits = torch.stack(logits, dim=1)  # [B, MAX_LEN, out_size]
    return logits

  def encode(self, imgs):
    images = self.cnn(imgs)  # [B, 512, H', W']
    images = images.permute(0, 2, 3, 1)  # [B, H', W', 512]
    B, H, W, _ = images.shape
    images = images.contiguous().view(B, H*W, -1)
    if self.add_pos_feat:
      images = add_positional_features(images)
    return images

  def step_decoding(self, states, o_t, enc_out, tgt):
    ...
    h_t, c_t = self.rnn(inp, states)  # h_t:[B, dec_rnn_h]
