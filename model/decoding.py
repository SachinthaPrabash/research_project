...
class LatexProducer(object):
  ...
  def __init__(self, model, vocab, beam_size=5, max_len=64, use_cuda=True):
    ...
    self.sign_to_id = vocab.sign_to_id
    self.id_to_sign = vocab.id_to_sign
  ...
  def _greedy_decoding(self, imgs):
    ...
    formulas_idx = torch.ones(batch_size, self.max_len, device=self.device).long() * PAD_TOKEN
    # first decoding step's input
    tgt = torch.ones(batch_size, 1, device=self.device).long() * START_TOKEN
    with torch.no_grad():
      for t in range(self.max_len):
        dec_states, O_t, logit = self.model.step_decoding(dec_states, O_t, enc_outs, tgt)

        tgt = torch.argmax(logit, dim=1, keepdim=True)
        formulas_idx[:, t:t + 1] = tgt
    results = self.idx_to_formulas(formulas_idx)
    return results

  def _simple_beam_search_decoding(self, imgs):
    """simpple beam search decoding (not support batch)"""
    self.model.eval()
    beam_results = [self._bs_decoding(img.unsqueeze(0)) for img in imgs ]
    return beam_results

  def idx_to_formulas(self, formulas_idx):
    ...
    for id_ in formulas_idx:
      ...
      for sign_id in id_list:
        if sign_id != END_TOKEN:
          result.append(self.id_to_sign[sign_id])
        else:
          break
  ...
  def _bs_decoding(self, img):
    ...
    topk_ids = torch.ones(self.beam_size, device=self.device).long() * START_TOKEN
    topk_log_probs = torch.Tensor([0.0] + [-1e10] * (self.beam_size - 1))
    topk_log_probs = topk_log_probs.to(self.device)
    seqs = torch.ones(self.beam_size, 1, device=self.device).long() * START_TOKEN
    # store complete sequences and corrosponing scores
    complete_seqs = []
    complete_seqs_scores = []
    k = self.beam_size
    vocab_size = len(self.sign_to_id)
    with torch.no_grad():
      for t in range(self.max_len):
        dec_states, O_t, logit = self.model.step_decoding(dec_states, O_t, enc_outs, topk_ids.unsqueeze(1))
        ...
        complete_inds = [ind for ind, next_word in enumerate(topk_ids) if next_word == END_TOKEN]
        ...
        O_t = O_t[seleted]
        dec_states = (dec_states[0][seleted], dec_states[1][seleted])

    i = complete_seqs_scores.index(max(complete_seqs_scores))
    seq = complete_seqs[i][1:]
    result = self.idx_to_formulas(seq.unsqueeze(0))[0]
    return result
  ...
  def _batch_beam_search(self, imgs):
    ...
    batch_size = imgs.size(0)
    start_predictions = torch.ones(batch_size, device=self.device).long() * START_TOKEN
    state = {}
    ...
    all_top_predictions = self.idx_to_formulas(all_top_predictions)
  ...
  def _take_step(self, last_predictions, state):
    ...
    with torch.no_grad():
      dec_states, O_t, logit = self.model.step_decoding(dec_states, O_t, enc_outs, last_predictions)
