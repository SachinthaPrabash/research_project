...
class Trainer(object):
  def __init__(self, optimizer, model, lr_scheduler, train_loader, val_loader, args, use_cuda=True, init_epoch=1, last_epoch=15):
  ...
  def train(self):
    ...
    while self.epoch <= self.last_epoch:
      self.model.train()
      losses = 0.0
      for imgs, target_for_training, target_for_cal_loss in self.train_loader:
        step_loss = self.train_step(imgs, target_for_training, target_for_cal_loss)
        losses += step_loss

        # log message
        if self.step % self.args.print_freq == 0:
          avg_loss = losses / self.args.print_freq
          print(mes.format(self.epoch, self.step, len(self.train_loader), 100 * self.step / len(self.train_loader), avg_loss, 2**avg_loss))
          losses = 0.0
  ...
  def train_step(self, imgs, target_for_training, target_for_cal_loss):
    self.optimizer.zero_grad()

    imgs = imgs.to(self.device)
    target_for_training = target_for_training.to(self.device)
    target_for_cal_loss = target_for_cal_loss.to(self.device)
    epsilon = cal_epsilon(
        self.args.decay_k, self.total_step, self.args.sample_method)
    logits = self.model(imgs, target_for_training, epsilon)

    # calculate loss
    loss = cal_loss(logits, target_for_cal_loss)
    self.step += 1
    self.total_step += 1
    loss.backward()
  ...
  def validate(self):
    ...
    with torch.no_grad():
      for imgs, target_for_training, target_for_cal_loss in self.val_loader:
        imgs = imgs.to(self.device)
        target_for_training = target_for_training.to(self.device)
        target_for_cal_loss = target_for_cal_loss.to(self.device)
